...
async function getProductConfig() {
    const product = await (0, product_model_1.getProductCollection)().findOne({});
    return {
        pricePerKit: product?.pricePerKit ?? 230,
        deliveryFeeInsideDhaka: product?.deliveryFeeInsideDhaka ?? 60,
        deliveryFeeOutsideDhaka: product?.deliveryFeeOutsideDhaka ?? 120,
        deliveryFeeThreshold: product?.deliveryFeeThreshold ?? 5,
    };
}
async function generateOrderId() {
    ...
    const counters = (0, db_1.getDB)().collection("counters");
    const result = await counters.findOneAndUpdate({ _id: `order_seq_${dateKey}` }, { $inc: { seq: 1 } }, { upsert: true, returnDocument: "after" });
...
async function createOrder(payload) {
    ...
    if (couponCode) {
        const couponResult = await (0, coupons_service_1.validateCoupon)(couponCode, subtotal);
        if (couponResult.success && couponResult.data) {
            discountAmount = couponResult.data.discountAmount;
            appliedCouponCode = couponResult.data.code;
        }
    }
    const newOrder = {
        orderId,
    ...
    await (0, orders_model_1.getOrdersCollection)().insertOne(newOrder);
    // Mark coupon as used
    if (appliedCouponCode) {
        (0, coupons_service_1.applyCouponUsage)(appliedCouponCode).catch(console.error);
    }
    const trackingToken = jsonwebtoken_1.default.sign({ orderId, purpose: "order_tracking" }, process.env.JWT_SECRET, { expiresIn: "30d" });
    if (email) {
...
async function sendAdminNewOrderEmail(adminEmail, order) {
    const html = `
  ...
  `;
    await (0, mailer_1.sendEmail)(adminEmail, `নতুন অর্ডার: ${order.orderId} — ম্যাগট-ফ্রি রেসকিউ কিট`, html);
}
async function sendOrderTrackingEmail(email, name, orderId) {
    const token = jsonwebtoken_1.default.sign({ orderId, purpose: "order_tracking" }, process.env.JWT_SECRET, { expiresIn: "30d" });
  ...
  `;
    await (0, mailer_1.sendEmail)(email, "Order Confirmation - Maggot Donation", html);
}
async function updateOrderStatus(id, status, reason) {
    ...
    const result = await (0, orders_model_1.getOrdersCollection)().findOneAndUpdate({ _id: new mongodb_1.ObjectId(id) }, {
    ...
    if (result.email) {
        sendStatusUpdateEmail(result.email, result.name, result.orderId, status.toLowerCase(), reason).catch(console.error);
    }
...
async function sendStatusUpdateEmail(email, name, orderId, status, reason) {
  ...
  `;
    await (0, mailer_1.sendEmail)(email, `অর্ডার আপডেট: ${orderId} — ${statusLabel}`, html);
}
async function getOrders(query) {
    const { page = "1", limit = "10", status, insideDhaka, orderDate, search, sortBy = "orderDate", sortOrder = "desc", } = query;
...
async function getOrderById(id) {
    ...
    const order = await (0, orders_model_1.getOrdersCollection)().findOne({ _id: new mongodb_1.ObjectId(id) });
    if (!order) {
        return { status: 404, success: false, message: "Order not found" };
    }
...
async function deleteOrder(id) {
    ...
    const result = await (0, orders_model_1.getOrdersCollection)().deleteOne({ _id: new mongodb_1.ObjectId(id) });
...
async function getOrderStats() {
    const collection = (0, orders_model_1.getOrdersCollection)();
    const [totalOrders, pendingOrders, approvedOrders, declinedOrders, cancelledOrders, completedOrders, insideDhakaOrders, outsideDhakaOrders, quantityResult,] = await Promise.all([
...
async function adminTrackOrder(id) {
    if (!id) {
        return { status: 400, success: false, message: "Order ID is required" };
    }
    const order = await (0, orders_model_1.getOrdersCollection)().findOne({ orderId: id });
    if (!order) {
        return { status: 404, success: false, message: "Order not found" };
    }
    return { status: 200, success: true, data: order };
}
async function trackOrder(id, token) {
    if (!id || !token) {
