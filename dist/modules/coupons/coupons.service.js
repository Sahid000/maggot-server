...
exports.applyCouponUsage = applyCouponUsage;
...
async function createCoupon(payload) {
    ...
    const existing = await (0, coupons_model_1.getCouponsCollection)().findOne({ code: code.toUpperCase().trim() });
    if (existing) {
    ...
    const result = await (0, coupons_model_1.getCouponsCollection)().insertOne(coupon);
...
async function getCoupons(query = {}) {
    ...
    const collection = (0, coupons_model_1.getCouponsCollection)();
...
async function toggleCoupon(id) {
    ...
    const coupon = await (0, coupons_model_1.getCouponsCollection)().findOne({ _id: new mongodb_1.ObjectId(id) });
    if (!coupon)
        return { status: 404, success: false, message: "Coupon not found" };
    const result = await (0, coupons_model_1.getCouponsCollection)().findOneAndUpdate({ _id: new mongodb_1.ObjectId(id) }, { $set: { isActive: !coupon.isActive, updatedAt: new Date() } }, { returnDocument: "after" });
...
async function deleteCoupon(id) {
    ...
    const result = await (0, coupons_model_1.getCouponsCollection)().deleteOne({ _id: new mongodb_1.ObjectId(id) });
...
async function validateCoupon(code, orderAmount) {
    if (!code)
        return { status: 400, success: false, message: "কুপন কোড প্রদান করুন" };
    const coupon = await (0, coupons_model_1.getCouponsCollection)().findOne({ code: code.toUpperCase().trim() });
    if (!coupon) {
...
async function applyCouponUsage(code) {
    await (0, coupons_model_1.getCouponsCollection)().updateOne({ code: code.toUpperCase().trim() }, { $inc: { usedCount: 1 }, $set: { updatedAt: new Date() } });
}
